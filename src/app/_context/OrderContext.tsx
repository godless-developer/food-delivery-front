...
import { FoodOrderType } from "@/types";
...
const OrderProvider = ({ children }: { children: ReactNode }) => {
  ...
  const fetchOrderData = useCallback(async () => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:4000/food-order/${currentUserId}`
      );
  ...
  const fetchAllOrderData = async () => {
    setLoading(true);
    try {
      const response = await axios.get("http://localhost:4000/food-order");
  ...
  const handleStatusChange = async (
    ...
    try {
      await axios.patch(`http://localhost:4000/food-order/${foodOrderId}`, {
        status: newStatus,
      });
      fetchAllOrderData();
    } catch (error) {
